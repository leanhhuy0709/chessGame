    ...
    for i in range(0, 9):
        for j in range(0, 9):
            ...
            if j == 0 and i > 0:
                ...
                continue  
            if (i + j) % 2 == 1:
                pg.draw.rect(screen, blue, ((100 + i * 60, 10 + j * 60), (60, 60)))
            else:
                pg.draw.rect(screen, white, ((100 + i * 60, 10 + j * 60), (60, 60)))
...
def Bot1():
    ...
    if board.is_checkmate():
        if board.move_stack.__len__() % 2 == 0:
            print("White win (Random)")
        else:
            print("Black win (Random)")
        Static.stop = True
def get_point_from_board():
    update_board()
    point = 0
    for i in range(0, 8):
        for j in range(0, 8):
